...
virtualenv_ansible:
	if [ "$(VENV_BASE)" ]; then \
		...
		if [ ! -d "$(VENV_BASE)/ansible" ]; then \
			...
			$(VENV_BASE)/ansible/bin/pip install $(PIP_OPTIONS) --ignore-installed setuptools==41.6.0 && \
			$(VENV_BASE)/ansible/bin/pip install $(PIP_OPTIONS) --ignore-installed pip==19.3.1; \
		fi; \
	fi
