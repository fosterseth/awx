...
class TaskManager():
    ...
    def start_task(self, task, rampart_group, dependent_tasks=None, instance=None):
        ...
        if not start_status:
            ...
            with disable_activity_stream():
                ...
                task.save()
                task.log_lifecycle(f"{task._meta.model_name}-{task.id} waiting",
    ...
    def generate_dependencies(self, undeped_tasks):
        ...
        for task in undeped_tasks:
            task.log_lifecycle(f"{task._meta.model_name}-{task.id} acknowledged",
                               extra={'type': task._meta.model_name,
                                      'template_name': task.unified_job_template.name,
                                      'task_id': task.id,
                                      'state': 'acknowledged'})
    ...
    def process_pending_tasks(self, pending_tasks):
        ...
        for task in pending_tasks:
            ...
            if blocked_by:
                ...
                task.log_lifecycle(f"{task._meta.model_name}-{task.id} blocked by {blocked_by._meta.model_name}-{blocked_by.id}",
                                   extra={'type': task._meta.model_name,
                                          'template_name': task.unified_job_template.name,
