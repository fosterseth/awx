...
class BaseTask(object):
    ...
    def pre_run_hook(self, instance, private_data_dir):
        ...
        '''
        instance.log_lifecycle(f"{instance._meta.model_name}-{instance.id} pre run", extra={'type': instance._meta.model_name, 'template_name': instance.unified_job_template.name, 'task_id': instance.id, 'state': 'pre_run'})

    def post_run_hook(self, instance, status):
        '''
        Hook for any steps to run before job/task is marked as complete.
        '''
        instance.log_lifecycle(f"{instance._meta.model_name}-{instance.id} post run", extra={'type': instance._meta.model_name, 'template_name': instance.unified_job_template.name, 'task_id': instance.id, 'state': 'post_run'})
    ...
    def final_run_hook(self, instance, status, private_data_dir, fact_modification_times, isolated_manager_instance=None):
        ...
        '''
        instance.log_lifecycle(f"{instance._meta.model_name}-{instance.id} finalize run", extra={'type': instance._meta.model_name, 'template_name': instance.unified_job_template.name, 'task_id': instance.id, 'state': 'finalize_run'})
    ...
    def run(self, pk, **kwargs):
        ...
        try:
            ...
            self.instance.log_lifecycle(f"{self.instance._meta.model_name}-{self.instance.id} running", extra={'type': self.instance._meta.model_name, 'template_name': self.instance.unified_job_template.name, 'task_id': self.instance.id, 'state': 'running'})
