    ...
    def delete(self):
        ...
        with transaction.atomic(using=self.using, savepoint=False):
            ...
            for model, instances_for_fieldvalues in self.field_updates.items():
                if self.logger:
                    self.logger.info("Updating fields for model %s", model.__name__)
            ...
            for qs in self.fast_deletes:
                if self.logger:
                    self.logger.info("Deleting fast deletes")
            ...
            for model, pk_list in del_dict.items():
                if self.logger:
                    self.logger.info("Deleting objects for model %s", model.__name__)
